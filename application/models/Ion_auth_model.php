...
class Ion_auth_model extends CI_Model
{
	...
	public function parent_registration($data){
	    $this->db->trans_begin();
	    
	    $this->db->insert('users',$data);
	    $uid = $this->db->insert_id();
	    $this->db->insert('users_groups',array('user_id'=>$uid,'group_id'=>4));
	...
	function registration($data){
		...
		$value['created_on'] = $data['created_on'];
		$value['ip_address'] = $data['ip_address'];
		...
		$output['school_id'] = $this->db->insert_id();
	 ...
	 * Teacher login 
	 **/
	public function teacher_login($identity, $password, $remember=FALSE){
	    ...
	    $this->db->select('u.'.$this->identity_column . ',u.school_id ,g.name as group, u.email, u.id, u.password,u.phone, u.active, u.last_login');
		...
		$query = $this->db->get($this->tables['users'].' u');
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$this->db->select('u.'.$this->identity_column . ',u.school_id,g.name as group, u.email, u.id, u.password, u.active, u.last_login,u.phone');
		$this->db->where($this->identity_column, $identity);
		$this->db->join('users_groups ug','ug.user_id = u.id');
		$this->db->join('groups g','g.id = ug.group_id');
		$this->db->limit(1);
	...
	public function set_session($user)
	{
		...
		$session_data = [
		    ...
		    'session'              => (int)$this->my_function->current_session(),
		    'last_check'           => time(),
		];
	...
	function check_school($str){
	    $this->db->select('school_code');
	    $this->db->limit(1);
	    $result = $this->db->get_where('school',array('school_code'=>$str,'status'=>1))->result_Array();
	    if(count($result)>0){
	        return true;
	    } else {
	        return false;
